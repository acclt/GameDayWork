...
```text
artifacts/GameDayWork-v0.3.1-win-x64-portable.zip
```
...
3. 从托盘进入黑屏，确认所有显示器均为纯黑；移动鼠标或按键，确认一秒内恢复。
4. 在黑屏时插拔显示器或调整显示布局，确认 Overlay 自动重建并覆盖所有屏幕。
5. 将一个无副作用测试程序设为短时定时任务，确认日志先记录 `PrepareAt`，Overlay 完全退出后在 `LaunchAt` 启动。
6. 用多个安全测试任务组成任务链，确认托盘状态在整条链期间持续为“执行任务”，结束后才回到空闲或黑屏。
7. 托盘选择“退出程序”，确认进程退出；普通关闭主窗口只应隐藏。

## BGI 安全确认

不要让验证流程自动启动 BGI。先使用无副作用测试程序验证 Screen Manager，再人工设置 BGI：观察日志中的“准备任务”与 Overlay 销毁记录，确认屏幕已恢复稳定后才手动允许到达 `LaunchAt`。BGI 运行期间托盘必须保持“执行任务”，期间 Screen Manager 不得重新黑屏。
